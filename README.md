...
先说一个最常被搞混的点：**GPLv3 约束的是"分发时给不给源码"，而不是"能不能商业化"。** 商业公司使用、甚至靠它盈利，本身完全自由且免费。

| 你的场景 | 是否需要开源你的代码 | 是否需要商业授权 |
|---|---|---|
| 公司内部用它管理生产数据库 | 否 | 否 |
| 修改后仅供公司内部使用，不对外发布 | 否 | 否 |
| 原样转发安装包给客户 / 同事 | 附带 GPLv3 与源码获取途径即可 | 否 |
| 分发修改版，或把本代码并入你的产品**并对外发布** | **是**，须以 GPLv3 开放完整源代码 | 否（若你接受开源） |
| 同上，但你的产品必须**闭源** | — | **是** |

具体条款：

- 你可以自由地运行、研究、修改和分发本软件，商业场景同样适用，**无需付费**。
- 但只要你**对外分发**（含修改版或包含本代码的衍生作品），就必须以同样的 GPLv3 条款开放对应的完整源代码，并保留版权与许可声明。
- **内部自用不受此限**——你自己或公司内部使用、修改而不对外分发，无需开源任何东西。
- 本软件按"原样"提供，不附带任何明示或暗示的担保。

> 只有当你需要把本项目集成进**闭源产品并对外分发**时，GPLv3 才成为障碍。这种情形可另行获取商业授权 —— 见下方[企业版与商业授权](#企业版与商业授权)。
